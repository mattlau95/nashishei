...
## 2026-07-01 — Production deploy executed: CORS fix, R2 storage, dead-code cleanup, Pages+Fly+Neon cutover (MAT-544/545/546/547/549/548, MAT-518)

**Session Goal:** Execute `docs/PRODUCTION-DEPLOY-PLAN.md` — take nashishei from docker-compose/localhost-only to a real public deployment (Cloudflare Pages + Fly.io + R2 + Neon), plus close the small backlog of prerequisite tickets the plan depended on.
**Status:** Completed ✅ — live and verified end-to-end at `https://nashishei.pages.dev` against the real Fly API, real R2 bucket, and real Neon database.

### The "Why" (Decision Log)

* **Pulled the `detect.go` dead-code deletion (MAT-547's scope) forward into the R2 storage work (MAT-545) rather than sequencing them separately:** `storage.Local.BasePath` was directly read by the dead `DetectImage` handler, which blocked defining a clean `Storage` interface. Deleting `DetectImage`/`callMLSidecar` (confirmed dead since the MAT-525 browser-ML pivot — only `detect-client` is live) was the minimal unblock. Left the rest of MAT-547 (`ml/` dir, compose service, `MLSidecarURL`) for its own pass.
* **`vite.config.ts` `rollupOptions.external` for `.wasm` rejected in favor of a `postbuild` deletion step:** tried marking `.wasm` external first, verified empirically it had zero effect on the build output — the oversized file is emitted via Vite's asset-URL pipeline (`new URL(..., import.meta.url)`), not Rollup's module-external mechanism, so `external` never had a chance of working. Confirmed via reading the minified bundle that the reference is a dead fallback branch (`wasmPaths ? ... : new URL(bundledAsset)`) — safe to delete post-build since the app always sets `wasmPaths` first.
* **Detached and destroyed the old Fly volume (`nashishei_storage`) in this session rather than leaving it as a follow-up:** `STORAGE_DRIVER=r2` already made it functionally unused; user confirmed its contents (Tauri-era test data) were disposable. Doing it live required two rounds of explicit confirmation for the actual `fly deploy`/`fly volumes destroy` steps — the auto-mode safety classifier correctly pushed back on `--yes`-style flag bypassing for an irreversible destroy until the user named the specific volume ID directly.
* **New Neon DB: dropped the (empty but goose-untracked) schema and re-ran `goose up` clean, over manually inserting `goose_db_version` rows:** the schema already matched the migrations exactly (verified by diff against `db/init.sql`), but goose's tracking table didn't know it. Manually forging tracking rows via raw SQL was flagged as bypassing the sanctioned migration tool; user chose the drop-and-remigrate path instead.
* **`useFaceCrops.ts` switched from same-origin-pathname fetching to fetching the full image URL directly:** the old approach assumed same-origin (`/files` via Vite proxy) to dodge CORS; this broke for real once photos moved to R2 — caught live via an incognito share-link test that 404'd against the Fly domain instead of R2. Both storage backends already send permitting CORS headers, so fetching the full URL directly needs no workaround.
* **Two Cloudflare Pages build failures, same symptom, different causes:** first was the genuine 25MiB oversized-WASM issue (fixed above). Second looked identical in the error log but was actually Pages retrying a stale pinned commit — user's best guess is Cloudflare was never granted full access to the repository during the initial "Connect to Git" flow, so post-creation pushes weren't reaching it. Confirmed the real cause by checking `origin/main` directly rather than trusting the dashboard error, then forced a fresh webhook with an empty commit once access was fixed.

### Technical Notes

* **Backend (`api/`):** `corsMiddleware` now takes an allowlist (`ALLOWED_ORIGINS`, comma-separated) instead of reflecting any `Origin`; adds `Vary: Origin`. Graceful shutdown via `http.Server` + `signal.NotifyContext(SIGINT, SIGTERM)` + `Shutdown(ctx)`, 10s timeout. New `storage.Storage` interface (`Save`/`URL`/`DeleteAll`); `storage.R2` implements it via `aws-sdk-go-v2/service/s3` against R2's S3-compatible endpoint (region `auto`); driver selected by `STORAGE_DRIVER` env (`local`/`r2`); `/files/*` route only registers under `local`. `config.go` gained `AllowedOrigins`/`StorageDriver`/`R2*` fields, lost `MLSidecarURL`.
* **Dead code removed:** `ml/` directory entirely (Python FastAPI sidecar + patch scripts + untracked `.venv/`), `docker-compose.yml`'s `ml` service, `detect.go`'s `DetectImage`/`callMLSidecar` + its route (kept `mlFace`/`detectionResult`/`computeSuggestions`/`floatsToVectorLiteral` — still used by the live `detect-client` path). `docs/updated-architechture.md` deleted (described the abandoned Tauri+sidecar architecture; already declared superseded by the plan doc).
* **Frontend build:** `VITE_ASSET_BASE` prefixes model URLs + `ort.env.wasm.wasmPaths` in `mlBrowser.ts`. `copy-wasm` fixed to also grab the `.mjs` glue files the JSEP/JSPI backends need (was `.wasm`-only, silently broken for any real production build until this session's testing caught it). New `postbuild` strips the dead bundled WASM file always, and additionally drops `dist/ort`/`dist/models` entirely when `VITE_ASSET_BASE` is set (R2 serves them instead). `Home.tsx` gained a missing `credentials: 'include'` on the image-list fetch (silent 401 only visible cross-origin).
* **Infra provisioned:** R2 bucket `nashishei-storage` populated with both ONNX models + all 8 onnxruntime-web WASM/`.mjs` runtime files at root keys matching the frontend's fetch paths; R2 defaults left `.mjs` files with no/wrong content-type (breaks strict ES-module MIME checking) — re-uploaded with explicit `--content-type`. Bucket CORS allows `GET,HEAD` from `nashishei.pages.dev`. Cloudflare Pages project `nashishei` connected to `mattlau95/nashishei` (root `frontend`, build `npm run build`, output `dist`). Fly secrets rotated/added: fresh `JWT_SECRET`, R2 credentials, `ALLOWED_ORIGINS`/`FRONTEND_URL` → `https://nashishei.pages.dev`, `DATABASE_URL` → the new Neon instance (migrated clean via `goose up`). `fly.toml`: dropped `STORAGE_PATH`, added `STORAGE_DRIVER = "r2"`, removed the `nashishei_storage` volume mount entirely.
* **Old Fly volume (`nashishei_storage`, 1GB) detached and destroyed** — confirmed unattached via `fly volumes list` before running `fly volumes destroy`, since STORAGE_DRIVER=r2 made it dead weight and its contents were confirmed-disposable Tauri-era test data.
* **Verified against the real public stack** (not local/mocked) via scripted Playwright: register → cookie survives reload (`SameSite=None; Secure`) → upload lands in R2 → client-side detection loads models/WASM from R2, finds faces → name a face → auto-generated share link renders correctly in a fresh incognito context (no cookies), face crops included.
* `go build`/`go vet`/`tsc -b --noEmit` clean throughout.

### Next Session

* Throwaway test accounts (`mat548-*@example.com`) sitting in the now-canonical Neon DB from live verification — low-priority cleanup.
